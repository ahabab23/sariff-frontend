...
// //     // Handle non-JSON responses (e.g. 404 with empty body)
// //     let data: any;
// //     try {
...
// //             // Update stored auth
// //             const rd = refreshResult.data;
// //             const newAuth: StoredAuth = {
// //               token: rd.accessToken,
// //               refreshToken: rd.refreshToken || auth.refreshToken,
// //               user: { ...auth.user, fullName: rd.name || auth.user.fullName },
// //               expiresAt: rd.expiresAt || auth.expiresAt,
// //             };
// //             localStorage.setItem("sariff_auth", JSON.stringify(newAuth));
...
// //         }
// //       } catch {
// //         // Refresh failed — fall through to return error
// //       }
// //     }
...
// // // Map enum names to numbers for backend
...
// //     method: "PUT",
// //     body: JSON.stringify({
// //       plan: typeof data.plan === "number" ? data.plan : planMap[data.plan] ?? 0,
// //       status:
...
//     body: JSON.stringify({
//       plan:
//         typeof data.plan === "number" ? data.plan : (planMap[data.plan] ?? 0),
//       status:
//         typeof data.status === "number"
//           ? data.status
//           : (statusMap[data.status] ?? 0),
...
// // ==================== TRANSACTION PIN (SuperAdmin) ====================
// export const setTransactionPin = (companyId: string, pin: string) =>
//   apiRequest<any>("/transaction-pin/set", {
//     method: "POST",
//     body: JSON.stringify({ pin }),
//   });

// export const toggleTransactionPin = (companyId: string) =>
//   apiRequest<any>("/transaction-pin/toggle", { method: "POST" });

// export const removeTransactionPin = (companyId: string) =>
//   apiRequest<any>("/transaction-pin", { method: "DELETE" });

// export const getTransactionPinStatus = () =>
//   apiRequest<any>("/transaction-pin/status");
const API_BASE =
  (process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080") + "/api";
...
// ==================== TRANSACTION PIN (SuperAdmin) ====================
export const setTransactionPin = (companyId: string, pin: string) =>
  apiRequest<any>("/transaction-pin/set", { method: "POST", body: JSON.stringify({ pin, companyId }) });

export const toggleTransactionPin = (companyId: string) =>
  apiRequest<any>(`/transaction-pin/toggle?companyId=${companyId}`, { method: "POST" });

export const removeTransactionPin = (companyId: string) =>
  apiRequest<any>(`/transaction-pin?companyId=${companyId}`, { method: "DELETE" });

export const getTransactionPinStatus = (companyId?: string) =>
  apiRequest<any>(`/transaction-pin/status${companyId ? `?companyId=${companyId}` : ''}`);