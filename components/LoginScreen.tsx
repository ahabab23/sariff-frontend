...
export function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  useEffect(() => {
    ...
    if (userId && userId.length > 0) {
      const userIdPattern = /^(SA|OU|CL)\d{3}$/i;
      if (!userIdPattern.test(userId)) {
        errors.userId = "Format: SA001, OU001, or CL001";
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user.phone !== telephone) {
      toast.error("Telephone number does not match User ID");
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (user.role === "client") {
        ...
        // Super Admin and Office Users need OTP
        setDetectedRole(user.role);
        setOtpStep(true);
        setIsLoading(false);
        toast.success(
          "Credentials verified! Please enter OTP sent to your phone."
        );
      }
    }, 1500);
  };
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (user) {
        toast.success(`Welcome back, ${user.name}!`);
        onLogin(detectedRole, user.name);
      }
    }, 1500);
  };
