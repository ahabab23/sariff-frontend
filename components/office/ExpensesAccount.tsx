...
export function ExpensesAccount() {
  ...
  // Map backend DTO to frontend ExpenseCategory interface
  const mapCategoryDto = (dto: any, index: number): ExpenseCategory => {
    return {
      id: dto.id,
      name: dto.name,
  ...
  // Map backend DTO to frontend ExpenseTransaction interface
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        const mappedCategories = response.data.map((dto, index) =>
          mapCategoryDto(dto, index)
        );
  ...
  const fetchExpenses = useCallback(async () => {
    try {
      const response = await getExpenses(1, 1000); // Get all expenses
      if (response.success && response.data) {
        const expenses = response.data.items || response.data;
        if (Array.isArray(expenses)) {
          const mappedExpenses = expenses.map(mapExpenseDto);
  ...
  useEffect(() => {
    ...
    loadData();
  }, [fetchCategories, fetchExpenses, fetchStats]);
  ...
  const handleCreateExpense = async (e: React.FormEvent) => {
    ...
    try {
      const createDto: CreateExpenseDto = {
        ...
        vendorPayee: formData.vendor || undefined,
      ...
      if (response.success) {
        const categoryName =
          ...
          "Expense";
        const currencyLabel = formData.currency === 0 ? "KES" : "USD";
        const paymentMethodNames: { [key: number]: string } = {
          0: "Cash at Hand",
          1: "Bank Transfer",
          2: "M-Pesa",
        ...
        toast.success(
          `Expense recorded! Double-entry applied:\nDebit: ${categoryName} (+${currencyLabel} ${
            formData.amount
          })\nCredit: ${
            paymentMethodNames[formData.paymentMethod]
          } (-${currencyLabel} ${formData.amount})`
        );
        setShowAddExpense(false);
        setFormData({
          categoryId: "",
  ...
  const getCategoryExpenses = (categoryId: string) => {
    return allExpenses.filter((exp) => exp.categoryId === categoryId);
  };
  ...
  // Calculate totals from stats or categories
  const totalKes =
  ...
  // Transaction filtering for selected category
  const categoryTransactions = selectedCategory
    ? getCategoryExpenses(selectedCategory.id)
    : [];
  const filteredTransactions = categoryTransactions.filter((txn) => {
    const matchesSearch =
        ...
        >
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex items-center gap-2 px-5 py-3 bg-slate-600 text-white font-bold shadow-lg hover:bg-slate-700 transition-all duration-300"
            >
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-200">
                {currentCategories.length === 0 ? (
                  ...
                  currentCategories.map((category, index) => {
                    const colors = getColorClasses(category.color);
                    const categoryExpenseCount = getCategoryExpenses(
                      category.id
                    ).length;
                    return (
                      <motion.tr
                      ...
                      >
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <span className="inline-flex items-center px-3 py-1 bg-slate-100 text-slate-700 border border-slate-200 font-bold text-xs">
                            {category.transactionCount || categoryExpenseCount}{" "}
                            expenses
                          </span>
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        ...
                        <p className="text-blue-100 text-sm flex items-center gap-1.5 mt-1">
                          <Tag className="w-3.5 h-3.5" />
                          {selectedCategory.id.slice(0, 8).toUpperCase()} •{" "}
                          {getCategoryExpenses(selectedCategory.id).length}{" "}
                          Expenses
                        </p>
                      </div>
                    </div>
              ...
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border-2 border-slate-200 bg-white text-slate-900 focus:border-purple-500 focus:outline-none transition-colors"
                  >
                    <option value={0}>Cash at Hand</option>
            ...
            >
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  toast.success(
                    `Expense ${selectedExpense.reference} updated successfully!`
                  );
                  setShowEditExpense(false);
                }}
                className="p-6 space-y-4 max-h-[70vh] overflow-y-auto"
              >
                ...
                <div>
                  ...
                  <select
                    defaultValue={selectedExpense.categoryId}
                    required
                    className="w-full px-4 py-3 border-2 border-slate-200 bg-white text-slate-900 focus:border-purple-500 focus:outline-none transition-colors"
                  >
                    <option value="">Select category...</option>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      defaultValue={selectedExpense.currency === "KES" ? 0 : 1}
                      required
                      className="w-full px-4 py-3 border-2 border-slate-200 bg-white text-slate-900 focus:border-purple-500 focus:outline-none transition-colors"
                    >
                      <option value={0}>KES</option>
