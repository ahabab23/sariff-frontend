  ...
  const handleExport = (format: ExportFormat) => {
    if (format === "pdf") {
      try {
        ...
        doc.setFontSize(8);
        doc.setFont("helvetica", "normal");
        doc.text("Period", pageWidth - 14, 28, { align: "right" });
        doc.setTextColor(17, 24, 39);
        doc.setFontSize(10);
        doc.setFont("helvetica", "bold");
        doc.text(selectedDate || "Today", pageWidth - 14, 35, {
          align: "right",
        });
        ...
        doc.setTextColor(107, 114, 128);
        doc.setFont("helvetica", "normal");
        doc.text("Money Out", 18 + cardWidth, yPos + 6);
        doc.setFontSize(11);
        ...
        doc.setTextColor(107, 114, 128);
        doc.setFont("helvetica", "normal");
        doc.text("Net Position", 18 + (cardWidth + 4) * 2, yPos + 6);
        doc.setFontSize(11);
        doc.setTextColor(
          netPositive ? 22 : 185,
          netPositive ? 163 : 28,
        ...
        doc.setTextColor(107, 114, 128);
        doc.setFont("helvetica", "normal");
        doc.text("Transactions", 18 + (cardWidth + 4) * 3, yPos + 6);
        doc.setFontSize(11);
        doc.setTextColor(17, 24, 39);
        doc.setFont("helvetica", "bold");
        doc.text(
        ...
        const balanceData = [
          [
            "Cash (KES)",
            `KES ${(
              dailyReport?.openingBalances?.cashKES || 0
            ).toLocaleString()}`,
            `KES ${(
              dailyReport?.closingBalances?.cashKES || 0
            ).toLocaleString()}`,
          ],
          [
            "Cash (USD)",
            `USD ${(
              dailyReport?.openingBalances?.cashUSD || 0
            ).toLocaleString()}`,
            `USD ${(
              dailyReport?.closingBalances?.cashUSD || 0
            ).toLocaleString()}`,
          ],
          [
            "Bank (KES)",
            `KES ${(
              dailyReport?.openingBalances?.bankKES || 0
            ).toLocaleString()}`,
            `KES ${(
              dailyReport?.closingBalances?.bankKES || 0
            ).toLocaleString()}`,
          ],
          [
            "Bank (USD)",
            `USD ${(
              dailyReport?.openingBalances?.bankUSD || 0
            ).toLocaleString()}`,
            `USD ${(
              dailyReport?.closingBalances?.bankUSD || 0
            ).toLocaleString()}`,
          ],
          [
            "M-Pesa",
            `KES ${(
              dailyReport?.openingBalances?.mpesa || 0
            ).toLocaleString()}`,
            `KES ${(
              dailyReport?.closingBalances?.mpesa || 0
            ).toLocaleString()}`,
          ],
        ];
        ...
        autoTable(doc, {
          startY: yPos + 4,
          head: [["Account", "Opening Balance", "Closing Balance"]],
          body: balanceData,
          theme: "plain",
        ...
        // ========== EXCHANGE SUMMARY (if applicable) ==========
        if (
          exchangeSummary &&
          (reportType === "exchange" || reportType === "daily")
        ) {
          ...
          doc.setFontSize(10);
          doc.setFont("helvetica", "bold");
          doc.text("EXCHANGE SUMMARY", 14, yPos);

          yPos += 6;
          const exchCardWidth = (pageWidth - 38) / 3;

          // USD Bought
          doc.setDrawColor(229, 231, 235);
          doc.rect(14, yPos, exchCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("USD Bought", 16, yPos + 6);
          doc.setFontSize(11);
          ...
          // USD Sold
          doc.rect(14 + exchCardWidth + 5, yPos, exchCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("USD Sold", 18 + exchCardWidth + 5, yPos + 6);
          doc.setFontSize(11);
          ...
          // Profit
          const profitPositive = (exchangeSummary.totalProfit || 0) >= 0;
          doc.rect(14 + (exchCardWidth + 5) * 2, yPos, exchCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Total Profit", 18 + (exchCardWidth + 5) * 2, yPos + 6);
          doc.setFontSize(11);
          doc.setTextColor(
        ...
        if (clientBalances.length > 0 && reportType === "clients") {
          const finalY = (doc as any).lastAutoTable?.finalY || yPos + 50;
          yPos = finalY + 12;
          ...
          doc.setFontSize(10);
          doc.setFont("helvetica", "bold");
          doc.text("CLIENT BALANCES", 14, yPos);

          const clientData = clientBalances.map((client) => [
            client.code,
            client.name,
          ...
          autoTable(doc, {
            startY: yPos + 4,
            head: [["Code", "Name", "Phone", "KES Balance", "USD Balance"]],
            body: clientData,
            theme: "plain",
            styles: { fontSize: 8, cellPadding: 3 },
            headStyles: {
        ...
        // ========== TRANSACTIONS TABLE (if applicable) ==========
        if (transactions.length > 0 && reportType === "transactions") {
          const finalY = (doc as any).lastAutoTable?.finalY || yPos + 50;
          yPos = finalY + 12;
          ...
          const txnData = transactions
            ...
            .map((txn: any) => [
              ...
              (txn.description || "").substring(0, 25) +
                ((txn.description?.length || 0) > 25 ? "..." : ""),
              txn.transactionType === 0 ? "DR" : "CR",
          ...
          autoTable(doc, {
            startY: yPos + 4,
            head: [["Date", "Code", "Description", "Type", "Curr", "Amount"]],
            body: txnData,
            theme: "plain",
            styles: { fontSize: 8, cellPadding: 3 },
            headStyles: {
        ...
        // ========== ACCOUNTS SUMMARY (if applicable) ==========
        if (accountSummary && reportType === "accounts") {
          const finalY = (doc as any).lastAutoTable?.finalY || yPos + 50;
          yPos = finalY + 12;
          ...
          doc.setFontSize(10);
          doc.setFont("helvetica", "bold");
          doc.text("ACCOUNTS SUMMARY", 14, yPos);

          yPos += 6;
          const acctCardWidth = (pageWidth - 38) / 3;

          // Total Cash
          doc.setDrawColor(229, 231, 235);
          doc.rect(14, yPos, acctCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Total Cash", 16, yPos + 6);
          doc.setFontSize(10);
          doc.setTextColor(17, 24, 39);
          doc.setFont("helvetica", "bold");
          doc.text(
          ...
          // Total Bank
          doc.rect(14 + acctCardWidth + 5, yPos, acctCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Total Bank", 18 + acctCardWidth + 5, yPos + 6);
          doc.setFontSize(10);
          doc.setTextColor(17, 24, 39);
          doc.setFont("helvetica", "bold");
          doc.text(
          ...
          // Total M-Pesa
          doc.rect(14 + (acctCardWidth + 5) * 2, yPos, acctCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Total M-Pesa", 18 + (acctCardWidth + 5) * 2, yPos + 6);
          doc.setFontSize(10);
          doc.setTextColor(17, 24, 39);
          doc.setFont("helvetica", "bold");
          doc.text(
        ...
        if (profitLossData && reportType === "profit") {
          const finalY = (doc as any).lastAutoTable?.finalY || yPos + 50;
          yPos = finalY + 12;
          ...
          yPos += 6;
          const plCardWidth = (pageWidth - 42) / 4;

          // Exchange Profit
          const exchProfitPositive = (profitLossData.exchangeProfit || 0) >= 0;
          doc.setDrawColor(229, 231, 235);
          doc.rect(14, yPos, plCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Exchange Profit", 16, yPos + 6);
          doc.setFontSize(10);
          doc.setTextColor(
          ...
          // Total Income
          doc.rect(14 + plCardWidth + 4, yPos, plCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Total Income", 18 + plCardWidth, yPos + 6);
          doc.setFontSize(10);
          doc.setTextColor(22, 163, 74);
          doc.setFont("helvetica", "bold");
          doc.text(
          ...
          // Total Expenses
          doc.rect(14 + (plCardWidth + 4) * 2, yPos, plCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Total Expenses", 18 + (plCardWidth + 4) * 2, yPos + 6);
          doc.setFontSize(10);
          doc.setTextColor(220, 38, 38);
          doc.setFont("helvetica", "bold");
          doc.text(
          ...
          // Net Profit
          const netProfitPositive = (profitLossData.netProfit || 0) >= 0;
          doc.rect(14 + (plCardWidth + 4) * 3, yPos, plCardWidth, 20, "S");
          doc.setFontSize(7);
          doc.setTextColor(107, 114, 128);
          doc.setFont("helvetica", "normal");
          doc.text("Net Profit", 18 + (plCardWidth + 4) * 3, yPos + 6);
          doc.setFontSize(10);
          doc.setTextColor(
        ...
        // ========== FOOTER ==========
        const lastY = (doc as any).lastAutoTable?.finalY || yPos + 30;
        if (lastY < pageHeight - 40) {
          doc.setDrawColor(229, 231, 235);
          doc.line(14, lastY + 12, pageWidth - 14, lastY + 12);
      ...
      if (reportType === "clients" && clientBalances.length > 0) {
        csvData = [
          [
            "Code",
            "Name",
          ...
          ...clientBalances.map((client) => [
            ...
            client.name,
            client.whatsAppNumber,
            client.balanceKES.toString(),
            client.balanceUSD.toString(),
            client.balanceType,
          ]),
        ];
      } else if (dailyReport) {
        csvData = [
          ["Metric", "Value"],
          ["Report Date", dailyReport.date || new Date().toLocaleDateString()],
          [
            "Total Credits",
            dailyReport.transactionSummary?.totalCredits?.toString() || "0",
          ],
          [
            "Total Debits",
            dailyReport.transactionSummary?.totalDebits?.toString() || "0",
          ],
          [
            "Transaction Count",
            dailyReport.transactionSummary?.transactionCount?.toString() || "0",
          ],
          [
            "Cash KES (Opening)",
            dailyReport.openingBalances?.cashKES?.toString() || "0",
          ],
          [
            "Cash KES (Closing)",
            dailyReport.closingBalances?.cashKES?.toString() || "0",
          ],
          [
            "Cash USD (Opening)",
            dailyReport.openingBalances?.cashUSD?.toString() || "0",
          ],
          [
            "Cash USD (Closing)",
            dailyReport.closingBalances?.cashUSD?.toString() || "0",
          ],
        ];
      }
      ...
      if (csvData.length > 0) {
        ...
        a.href = url;
        a.download = `report_${reportType}_${
          new Date().toISOString().split("T")[0]
        }.${format === "csv" ? "csv" : "xls"}`;
        a.click();
